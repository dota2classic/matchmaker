...
**Before publishing, make sure you are logged into npm and the scope org exists:**

```bash
npm whoami        # should print your username; if not, run: npm login
```

The scoped package (e.g. `@yourorg/your-api-generated`) requires the `@yourorg` organization to already exist on npmjs.com and your account to have publish access. Create it at npmjs.com if needed.

> **Note:** Do not set `provenance: true` in `publishConfig` — provenance is only supported in CI (GitHub Actions) where an OIDC token is available. Setting it locally causes `EUSAGE: Automatic provenance generation not supported for provider: null`. The `--provenance` flag in the workflow handles this for CI publishes.
